# ---
# Path and Filename: JIT_ATL/Scripts/train_micro_corpus_heavy.py
# Last Updated: 2026-03-12 13:11:27 +10:00
# ---
...
def build_heavy_corpus():
    print("-> Constructing the Heavy Repetition Micro-Corpus...")
    ...
    raw_triplets = [
        ...
        # 6. THE HIGHWAY (Triangulation Triplets)
...
def execute_heavy_paving():
    print("=== JIT_ATL: PHASE 5 - KINETIC MASS INJECTION (HEAVY REPETITION) ===\n")
    
    model_name = 'all-MiniLM-L6-v2'
    print(f"Loading Model A ({model_name})...")
    model = SentenceTransformer(model_name)
    
    train_examples = build_heavy_corpus()
    train_dataloader = DataLoader(train_examples, shuffle=True, batch_size=16)
    train_loss = losses.CosineSimilarityLoss(model)
    
    print("\n-> Initiating Heavy Repetition Fine-Tuning...")
    ...
    output_path = "./JIT_ATL/Model_B_Heavy"
    os.makedirs(output_path, exist_ok=True)
    model.save(output_path)
    
    print(f"\n-> SUCCESS: Model B (Heavy) saved to {output_path}")
    print(f"-> Heavy Training Duration: {execution_time:.2f} seconds")

if __name__ == "__main__":
    execute_heavy_paving()
